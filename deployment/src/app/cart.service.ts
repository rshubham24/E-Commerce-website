...
export class CartService {
  ...
  addToCart(prod: CartModel) {
    return this.http.post(Backend_Url + "/cart/add", prod);
  }

  getCartItem(customerId: string) {
    this.http.get<{message: string, products: any}>(Backend_Url + "/cart/get_cart_item/" + customerId)
  ...
  updateItem(item: CartIdModel) {
    this.http.put(Backend_Url + "/cart/update/" + item.id, item).subscribe(response => {
  ...
  placeOrder(cartItems: CartIdModel[], mobile: string, streetAdress: string, city: string, state: string, country: string, pinCode: number, totalPrice: number) {
    ...
    return this.http.post(Backend_Url + "/order/place", order);
  }

  deleteItem(productId: string){
    return this.http.delete(Backend_Url + "/cart/delete/" + productId);
  }


  deleteAllItem(customerId: string) {
    return this.http.delete(Backend_Url + "/cart/delete_all/" + customerId);
  }

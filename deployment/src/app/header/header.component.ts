...
export class HeaderComponent implements OnInit, OnDestroy{
  ...
  redirectToCart() {
    if(this.customerIsAuthenticated){
      this.router.navigate(['/shopping_cart/' + localStorage.getItem("userId")]);
    }
  }

  redirectToMyOrders() {
    if(this.customerIsAuthenticated){
      this.router.navigate(['/customer_orders/' + localStorage.getItem("userId")]);
    }
  }

  redirectToManageOrders()  {
    if(this.retailerIsAuthenticated){
      this.router.navigate(['/manage/' + localStorage.getItem("userId")]);
    }
  }
